...
# TODO: 3.使用Normalizer对样本的特征X进行数据预处理
scaler5 = preprocessing.Normalizer().fit(X_train)
X5_train = scaler5.transform(X_train)
X5_test = scaler5.transform(X_test)

# TODO: 4.输出RobustScaler处理后的数据统计信息, 给出样本两个特征的极值
print("Normalizer, 训练集特征一的最大值：{:.2f}， 最小值：{:.2f}。".format(
    np.max(X5_train[:, 0]), np.min(X5_train[:, 0])))
print("Normalizer, 训练集特征二的最大值：{:.2f}， 最小值：{:.2f}。".format(
    np.max(X5_train[:, 1]), np.min(X5_train[:, 1])))
print("Normalizer, 测试集特征一的最大值：{:.2f}， 最小值：{:.2f}。".format(
    np.max(X5_test[:, 0]), np.min(X5_test[:, 0])))
print("Normalizer, 测试集特征二的最大值：{:.2f}， 最小值：{:.2f}。".format(
    np.max(X5_test[:, 1]), np.min(X5_test[:, 1])))
...
plt.subplot(122)
# 绘制样本的散点图
plt.scatter(X5_train[:, 0], X5_train[:, 1],
...
plt.legend(loc='best')
plt.savefig('Codes//Ch09Preprocessing//results//Ch0905Normalizer.png', dpi=150)
plt.show()
