...
# TODO: 4.输出MinMaxScaler处理后的数据统计信息, 给出样本两个特征的极值
print("MinMaxScaler, 训练集特征一的最大值：{:.2f}， 最小值：{:.2f}。".format(
    np.max(X3_train[:, 0]), np.min(X3_train[:, 0])))
print("MinMaxScaler, 训练集特征二的最大值：{:.2f}， 最小值：{:.2f}。".format(
    np.max(X3_train[:, 1]), np.min(X3_train[:, 1])))
print("MinMaxScaler, 测试集特征一的最大值：{:.2f}， 最小值：{:.2f}。".format(
    np.max(X3_test[:, 0]), np.min(X3_test[:, 0])))
print("MinMaxScaler, 测试集特征二的最大值：{:.2f}， 最小值：{:.2f}。".format(
    np.max(X3_test[:, 1]), np.min(X3_test[:, 1])))
...
plt.subplot(122)
# 绘制样本的散点图
plt.scatter(X3_train[:, 0], X3_train[:, 1],
            c='c', cmap=plt.cm.cool, label='训练集')
plt.scatter(X3_test[:, 0], X3_test[:, 1], c='b', cmap=plt.cm.cool, label='测试集')
plt.title('图3: MaxAbsScaler')


plt.legend(loc='best')
plt.savefig('Codes//Ch09Preprocessing//results//Ch0903MaxAbsScaler.png', dpi=150)
plt.show()
