...
# 训练集文件
train_images_idx3_ubyte_file = os.path.join(
    rootPath, 'train-images.idx3-ubyte')
# 训练集标签文件
train_labels_idx1_ubyte_file = os.path.join(
    rootPath, 'train-labels.idx1-ubyte')
...
def decode_idx3_ubyte(dataname, idx3_ubyte_file):
    ...
    for i in range(num_images):
        if (i + 1) % 10000 == 0:
            print('\r {0}图片大小: {1}*{2}, 已载入{3}/{4}.'.format(dataname,
                                                           num_rows, num_cols, i + 1, num_images), end='')
            # print(offset)
